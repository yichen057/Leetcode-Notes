...
class Solution:
    ...
    # Complexity Analysis
# Time Complexity: O(NKlogK), where N is the length of strs, and K is the maximum length of a sorted string in strs. The outer loop has complexity O(N) as we iterate through each string. Then, we sort each string in O(KlogK) time.
